...
async def enviar_comprovativo(aluno_nome: str):
    aluno_normalizado = aluno_nome.strip().lower()
    html = f"""
    <!DOCTYPE html>
    <html lang="pt">
    ...
    <body>
      ...
      <div class="container">
        <h1>Enviar Comprovativo de Pagamento</h1>
        <p id="nomeAluno">Aluno: {aluno_nome}</p>

        <form id="formComprovativo">
          <input type="file" id="comprovativo" name="comprovativo" accept="image/*,application/pdf" required>
          <br>
      ...
      <script>
        const alunoNome = "{aluno_normalizado}";
        const form = document.getElementById("formComprovativo");
        ...
        form.addEventListener("submit", async (e) => {{
          ...
          try {{
            const res = await fetch("/upload_comprovativo", {{
              method: "POST",
              body: formData
            }});

            if (res.ok) {{
              mensagem.textContent = "Comprovativo enviado com sucesso!";
              fileInput.value = "";
            }} else {{
              mensagem.textContent = "Falha ao enviar o comprovativo.";
